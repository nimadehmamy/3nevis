...
function getDistz(){
  for(var i in distz){
    ds0[i]=0 ;
    for(var j in distz[i]){ ds0[i] += (distz[i][j] > 1e-20) && (distz[i][j] < controls.edgeDiameter) }
...
function colorEdges(){
  ...
  for(var i in edge_list){
    ...
    var c = ds0[i] ? 0 : 1; //Math.pow(1 - ds0[i]/mx, 10) ;
    l.link.material.color.setRGB(1,c,c);
  }
}
...
var Viz = function(){
  ...
  //try
  {
    var j = $.getJSON('assets/network/'+netName+'/info.json', function(data){
      //console.log(data);
      network.info = data;
      try{
...
var sh= shell(Math.abs(controls.scale)/2 +1 ,1);
